...
int 	HD_NUM = 100;  	 //SNP number in high density area for ervry turmor
float 	HD_POS = 0.5;  	 //position in reference for high density area of ervry turmor
int 	HD_LEN = 100000; //100000; length of high density area
...
enum muttype_t {NOCHANGE = 0, INSERT = 0x80, SUBSTITUTE = 0x40, DELETE = 0xc0, GTYPEBB=0x20, GTYPEAB=0x10, GTYPEAA=0x30, LONGINSERT = 0xf0000000 };
...
int set_pos_sim(mutseq_t *hap1, mutseq_t *hap2, seq_t *nochged, int k){  // return chged num
	...
	c1 = ret[1]->s[k];
	chgnum = 1;
	if (zrand(MAXSS) >= INDEL_FRAC) { // substitution
		c = (c0 + (int)(zrand(MAXSS)* 3.0 + 1)) & 3;
		if (zrand(MAXSS) < BB_RATE) { // hom
			ret[0]->s[k] = SUBSTITUTE|GTYPEBB|c;
			ret[1]->s[k] = SUBSTITUTE|GTYPEBB|c;
		} else { // het
			tmp = zrand(MAXSS)<0.5?0:1;
			ret[tmp]->s[k] = SUBSTITUTE|GTYPEAB|c;
		}
	} else { // indel
		if (zrand(MAXSS) < DEL_RATE) { // deletion
			if (zrand(MAXSS) < BB_RATE ) { // hom-del
				ret[0]->s[k] |= DELETE|GTYPEBB;
				ret[1]->s[k] |= DELETE|GTYPEBB;
				deleting = 3;
			} else { // het-del
				deleting = zrand(MAXSS)<0.5?1:2;
				ret[deleting-1]->s[k] |= DELETE|GTYPEAB;
			}
			k++;
			c0 = ret[0]->s[k];
			c1 = ret[1]->s[k];
			while(zrand(MAXSS) < INDEL_EXTEND  && k < max && c0<4 && c1<4 && nochged->s[k]==0 ) {
			...
			do {
				num_ins++;
			       	ins = (ins << 2) | (int)(zrand(MAXSS) * 4.0);
			} while (num_ins < 10 && zrand(MAXSS) < INDEL_EXTEND);
			chgnum = num_ins; //insert num counting;
			if (zrand(MAXSS) < BB_RATE ) { // hom-ins
...
/*******************************************************/
void set_mut_sim(seq_t *seq, mutseq_t *hap1, mutseq_t *hap2, seq_t *nochged, mutseq_t *norab)
{
	mutseq_t *ret[2];
	int num, j, k, n, m, max;
	double r;
	...
	hdbeg = max*HD_POS; //High density area begenning
	printf("Enter set_mut_sim max=%d   num=%d \n", max, num);
	n = 0; 
	while(n < num){ //set mutation and not normal AB mode, not High density area
		k = zrand(MAXSS)*max;
		c0 = ret[0]->s[k];
		c1 = ret[1]->s[k];
		while (c0 >= 4 || c1 >= 4 || nochged->s[k] || (HD_LEN>0)&&(k>hdbeg && k<hdbeg+HD_LEN)){
			k = zrand(MAXSS)*max;
			c0 = ret[0]->s[k];
			c1 = ret[1]->s[k];
		}
	...
	printf("\tEnter High density area set:  HD_NUM=%d \n", HD_NUM);
	n = 0; 
	while(n < HD_NUM){ //set mutation in High density area
		k = hdbeg + zrand(MAXSS)*HD_LEN; // random in High density area
		c0 = ret[0]->s[k];
		c1 = ret[1]->s[k];
		while (c0 >= 4 || c1 >= 4 || nochged->s[k]){
			k = hdbeg + zrand(MAXSS)*HD_LEN;
			c0 = ret[0]->s[k];
			c1 = ret[1]->s[k];
		}
		nochged->s[k] |= 4;
	...
	for (j=0; j<num; j++){  // mutation from normal AB mode 
		...
		if (m<10){
			c0 = ret[0]->s[n]; 
			c1 = ret[1]->s[n];
			if (zrand(MAXSS) < BB_RATE) { //AB==>BB
				if ((c0 & 0x30) == GTYPEAB){ 
					c = (c0 & 0x3 );
				} else c = (c1 & 0x3 );
				ret[0]->s[n] = ret[1]->s[n] = SUBSTITUTE|GTYPEBB|c; 
			}else{  //AB==>AA
				c = (mut_t)gch_to_num(seq->s[n]); // get reference char
				ret[0]->s[n] = ret[1]->s[n] = SUBSTITUTE|GTYPEAA|c; 
			}
			nochged->s[n] |= 4;
...
/*******************************************************/
void init_del_sim( seq_t *seq, mutseq_t *hap1, mutseq_t *hap2, seq_t *nochged, mutseq_t *norab , indel_t *indelp, char *simfile)
{
	int fret, no, idno, idbeg, idlen, idtype, idnum, idvar;
	char ch=0, name[50], *idstr, *idmark, base_simidxf[50]; //[100000];
	int i,j,k, n1, n2, h, del_no, in_no;
	mutseq_t *ret[2];
	int tmp, n, pos;
	double r;
	mut_t s0, s1;
	...
	while ( fscanf(indelfp, "%d %d %d %d %d %d %s\n", &idno, &idbeg, &idlen, &idtype, &idnum, &idvar, idstr) != EOF ){
		if (idlen<0) {  // long delete from long indel file!!!
			...
			case 1: if (zrand(MAXSS)<0.5) for (int i=0; i<idlen; i++) ret[0]->s[idbeg+i] |= DELETE|GTYPEAB;
				...
				break;
			case 2:
				for (int i=0; i<idlen; i++){ ret[1]->s[idbeg+i] = ret[0]->s[idbeg+i] |= DELETE|GTYPEBB;}
				n1 = n1 + idlen;
				del_no ++;
				break;
			default: break;
			...
			if (idbeg<0 || idnum<1) idtype=-1;
			else if (idbeg+idlen-1 >= REF_LEN) idtype=-1;
			switch(idtype){
			case 0: break;
			case 1: h = (int)(zrand(MAXSS)*2);
				ret[h]->s[idbeg] = (ret[h]->s[idbeg] & 0x03 );
				ret[h]->s[idbeg] |= (LONGINSERT|INSERT|GTYPEAB|(idno<<8));
				break;
			case 2: 
				ret[0]->s[idbeg] = (ret[0]->s[idbeg] & 0x03 );
				ret[1]->s[idbeg] = (ret[1]->s[idbeg] & 0x03 );
				ret[0]->s[idbeg] |= (LONGINSERT|INSERT|GTYPEBB|(idno<<8));
				ret[1]->s[idbeg] |= (LONGINSERT|INSERT|GTYPEBB|(idno<<8));
				break;
			default: break;
			}
			if (idtype > 0) {
				...
				if (idvar == 0){  //repeat without var genos
					for(i=0; i<idnum; i++) addinsert_iterm(indelp, idno, idbeg, idlen, idtype, idnum, idvar, idstr, &n2);
				}else { //repeat with var genos
					for(i=0; i<strlen(idstr); i++) idmark[i] = 0;
					res_num = idnum;
					while(res_num>4){
						k = zrand(MAXSS)*(0.5*res_num-1)+1;
						for(i=0; i<k; i++) addinsert_iterm(indelp, idno, idbeg, idlen, idtype, idnum, idvar, idstr, &n2);
						res_num = res_num - k;
					...
					if (res_num>1){
						...
						i = (gch_to_num(idstr[j]) + (int)(zrand(MAXSS)* 3.0 + 1)) & 3;
						idstr[j] = num_to_gch(i);
						for(i=0; i<res_num; i++) addinsert_iterm(indelp, idno, idbeg, idlen, idtype, idnum, idvar, idstr, &n2);
					}else if (res_num == 1) addinsert_iterm(indelp, idno, idbeg, idlen, idtype, idnum, idvar, idstr, &n2);
				}//if(ifvar ==0 )...
				in_no++;
...
static int simu_usage()
{
	...
	printf("         -p FLOAT      	the position of high desity [%f]\n", HD_POS);//0.5
	printf("         -n int      	mutation number in high desity [%d]\n", HD_NUM);//120
	printf("         -l int       	length of high desity [%d]\n", HD_LEN); //100000
...
{	char c, cc, fhead[50];
	...
	srand48(time(0));  // Init Random data 
	outresultf[0]='\0';
	indelf[0]='\0';
	cc=0;
	while ((c = getopt(argc, argv, "r:R:X:D:B:A:n:p:l:I:N:C:o:")) >= 0) {
		switch (c) {
		...
		case 'n': HD_NUM = atoi(optarg);      if (HD_NUM<0 || HD_NUM>HD_LEN) cc =1; break;
		case 'p': HD_POS = atof(optarg);      if (HD_POS<0 || HD_POS>1) cc =1; break;
		case 'l': HD_LEN = atoi(optarg);      if (HD_LEN<0) cc =1; break;
	...
	init_del_sim(&seq, &sim[0], &sim[1], &nochged, &norab, &indelp, insimf );
	printf("init_del_sim() return!\n\n");

	//* set mutation for simsulator */
	set_mut_sim(&seq, &sim[0], &sim[1], &nochged, &norab );
